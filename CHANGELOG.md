...
### Added
- **Whole-swarm hibernation on a Claude rate limit** — when any agent hits a Claude usage/session limit the *entire swarm* now pauses instead of pausing that one agent. `AgentPool.enter_hibernation(reason, trigger_agent_id)` flips a process-wide `_hibernating` flag, kills every in-flight agent (driven off `db.get_running_agents()` so reattached post-restart survivors monitored by `_monitor_pid` are swept too), and resets each killed agent's work item via `_hibernate_one` so the normal pollers re-dispatch it from scratch once the limit clears. `wake_from_hibernation()` clears the flag and lets dispatch resume. New `is_hibernating` / `can_dispatch` properties gate all new work; the issue poller (`main._poll_and_dispatch`), `PRMonitor`, and `ConflictMonitor` all early-return while hibernating
- **Hibernation survives an orchestrator restart** — `enter_hibernation` writes a sentinel file (`HIBERNATION_STATE_FILE`, default `<workspaces>/.hibernating`) holding the UTC timestamp and reason; `AgentPool.__init__` restores the paused state from it on startup, so a restart mid-hibernation stays paused until the watcher confirms the reset. `wake_from_hibernation` removes the sentinel
- **`RateLimitWatcher` rewritten to drive hibernation** (`rate_limit_watcher.py`) — while the swarm is hibernating it probes Claude every `RATE_LIMIT_RETRY_INTERVAL` seconds and calls `wake_from_hibernation()` once Claude responds, then falls back to the legacy `rate_limited` resume path only for any pre-hibernation rows left over from before this system existed
- **Live UI indication that the swarm is rate-limited** — `GET /api/metrics` now returns a `hibernation` object (`{hibernating, since, reason}`) sourced from `AgentPool.hibernation_status()` (reads the sentinel). The dashboard renders a new `HibernationBanner` component (`frontend/src/components/metrics/HibernationBanner.jsx`) that explains the swarm is paused, why, and for how long; the "Rate Limited" metric card shows `PAUSED` while hibernating (paused agents are marked `hibernated`, not `rate_limited`, so the raw count alone read a misleading `0`)
- **Manual "Check rate limit" probe** — `POST /api/rate-limit/check` runs `AgentPool.manual_probe()` (a fresh `claude -p` probe, executed off the event loop via `asyncio.to_thread`) and wakes the swarm immediately if the limit has cleared, so an operator no longer has to wait up to 15 minutes for the watcher's next automatic poll. Surfaced as a button in the hibernation banner with inline result feedback (`Limit cleared — resuming` / `Still rate-limited`)
- **`AgentPool._confirm_rate_limited()` + shared `_probe_claude_available()`** — because hibernation kills *every* agent, a suspected limit is confirmed with a live probe first, so an unrelated error whose text merely matches a rate-limit pattern (an agent working on HTTP 429 handling, a disk "capacity" error, a log line containing "429") falls through to the normal failure path rather than pausing the whole swarm. The probe lives in `agent_pool.py` and is reused by the watcher
- **`db.get_hibernated_agents()`** — lists agents killed by a hibernation sweep (`status = 'hibernated'`)

### Fixed
- **Hibernation never actually triggered for the most common limit — the swarm retried forever instead of pausing** — the Claude CLI announces the 5-hour window as *"You've hit your session limit · resets &lt;time&gt;"*, but `_RATE_LIMIT_PATTERNS` had no `session limit` token, so the message matched nothing. The agent fell through to the normal-failure path, the issue was reset to `pending`, the poller re-dispatched it, and it hit the same limit again — the tight FAILED→retry loop visible on the dashboard. Added `session limit` (and `limit reached`) to the pattern list
- **Rate-limit text in `result` / `assistant` / `rate_limit_event` stream events was ignored** — `_is_rate_limit_error` scanned only `stderr` and `error`-type events, but the CLI delivers the session-limit notice as a dedicated `rate_limit_event` *and* as the terminal `result` / an `assistant` turn (often with empty stderr and exit 0). It now treats any `rate_limit_event` as definitive and also scans `result` and `assistant` summaries; the broadened scan is safe because every trigger is still gated on the live `_confirm_rate_limited()` probe
- **The confirmation probe (and the wake-watcher) couldn't recognize a session limit either** — `_probe_claude_available()` returned `True` on exit 0 *before* inspecting the output, and otherwise matched the same incomplete pattern set, so even with detection fixed it would report Claude "available" during a real session limit (never confirming hibernation, and flapping the watcher awake). It now scans combined stdout+stderr for a limit signature *before* trusting the exit code
- **A rate limit that exits 0 was never evaluated** — in `_monitor_agent` the rate-limit branch sat *after* the `return_code == 0` success check, so a limit the CLI reported on a clean exit was treated as a successful-but-empty run (issue marked failed, then re-dispatched). The detection now runs *before* the success path
- **Reattached agents (post-restart) had no rate-limit detection at all** — `_monitor_pid`, the completion path for agents that survived an orchestrator restart, never checked for a limit. Added `_reattached_hit_rate_limit(agent_id)` (scans the persisted `agent_events` rows the same way `_is_rate_limit_error` scans live events) and a confirm-gated `enter_hibernation` call
- **Hibernation banner rendered literal `⏸` / `—`** — the escape sequences were written into JSX *text nodes*, where `\uXXXX` is not interpreted, so they printed verbatim (the pre-existing `ErrorBanner`'s `—` had the same latent bug). Replaced with real lucide-react icons and HTML entities

### Changed
- **Per-work-item retry budgets are rolled back on a hibernation pause** — `_hibernate_one` decrements an implement issue's `attempts` by one (the fresh dispatch on wake re-increments it) and flips a `resolve_conflict` agent's recorded conflict-fix attempt to `dispatch_failed`, so a rate-limit pause (not the item's fault) doesn't burn its retry budget and prematurely escalate to a human. Hibernated worktrees are removed so the restart begins from a clean checkout
- **Default `RATE_LIMIT_RETRY_INTERVAL` raised to `900`s** (15 min) — the cadence at which the watcher probes Claude while the swarm is hibernating
...
### Fixed
- **`GH_TOKEN` rotation now actually rotates** — when an operator updated `GH_TOKEN` in `.env` and restarted, `git fetch` in existing workspaces still failed with `remote: Invalid username or token. Password authentication is not supported for Git operations.`, breaking issue dispatch (`Failed to create worktree for issue #N: git command failed: git -C <workspace> fetch origin`). Two layers of per-repo state were pinning the old token: `credential.helper` (baked into `.git/config` at clone time by `_clone_repo_thread`) and an embedded `https://x-access-token:<old>@github.com/...` in `remote.origin.url` (left behind by older clone-path code that never stripped it). The URL-embedded form silently overrides the credential helper, so refreshing only the helper was not enough — both have to be reconciled. New `configure_repo_auth(local_path)` in `workspace_manager.py` strips any `https://<creds>@` prefix from `remote.origin.url` via `re.sub(r"https://[^@/]+@", "https://", ...)` and rewrites `credential.helper` to a shell function that emits the current in-process `GH_TOKEN`; idempotent (no-op when URL is already clean and helper is already current), 10s timeouts on every subprocess call, early-returns when `GH_TOKEN` is empty so SSH-only setups are not disturbed. Called at the top of every network op in `worktree.py` (`ensure_repo_updated`, `get_sync_status`, `create_worktree_for_pr`); pushes and `git ls-remote` calls from agent worktrees in `agent_pool.py` inherit the parent repo's `credential.helper` (confirmed via `git config --show-origin` resolving to the parent's `.git/config`), so they are covered transitively without per-call-site wiring
- **Agents stopped creating PRs after invoking the `commit-commands:commit` skill** — with `SKILLS_ENABLED=true` (default since 1.0.14) the user-installed `commit-commands` plugin exposes a `commit` skill whose `SKILL.md` body ends with *"Do not use any other tools or do anything else. Do not send any other text or messages besides these tool calls."* Implement agents reasonably picked it for Step 4 of the prompt ("Commit and push"), it ran the commit, and the stop-instruction terminated the agent (`stop_reason: end_turn`, empty `result`) before Step 5 (`git push`), Step 6 (`gh pr create`), or Step 7 (label the PR). The salvage path added in 1.4.9 (`_salvage_implement_work`) would then auto-create a generic `Fix #N: Auto-created from agent work` PR, masking the regression behind a successful-looking dashboard row. `_skills_block` in `prompts.py` now always emits when `SKILLS_ENABLED=true` — even when `_discover_installed_skills()` returns an empty list, because plugin-marketplace skills under `~/.claude/plugins/marketplaces/.../commands/` are loaded by the `claude` CLI at runtime regardless of what the prompt lists — a hard-block warning naming `commit-commands:commit`, `commit-commands:commit-push-pr`, and `commit-commands:clean_gone` and instructing the agent to run `git` / `gh` directly via the Bash tool exactly as the numbered steps specify. Inherited by all four prompt builders (`build_implement_prompt`, `build_fix_review_prompt`, `build_resume_implement_prompt`, `build_resume_fix_review_prompt`)
- **`build_resume_implement_prompt` raised `NameError: name 'owner' is not defined` on every rate-limit resume of an implement agent** — the f-string body referenced `{owner}` and `{repo_name}` in the Step 7 label-fetch instructions (`gh api repos/{owner}/{repo_name}/labels`, `gh api repos/{owner}/{repo_name}/issues/PR_NUMBER/labels`) but neither was defined as a local; `github_repo` was being passed in but never split. Any rate-limit pause followed by a resume would crash inside prompt construction before the new agent process could be spawned, silently stranding the preserved worktree until manual intervention. Fixed by adding the same `repo = github_repo; owner, repo_name = repo.split("/", 1)` lines that `build_implement_prompt` and `build_resume_fix_review_prompt` already had at the top of their bodies

### Added
- **`configure_repo_auth(local_path)`** in `workspace_manager.py` — extracts the post-clone credential-setup logic from `_clone_repo_thread` into a reusable helper that normalizes per-repo git auth state to the current `GH_TOKEN`. Strips any embedded `https://<user>:<pass>@` prefix from `remote.origin.url` and rewrites `credential.helper` with a shell function emitting `username=x-access-token` and `password=<GH_TOKEN>`. Safe to call from any thread and any number of times: concurrent calls on the same repo write identical values, and `git config` writes atomically via temp-file rename so racing on the helper cannot corrupt `.git/config`
...
- **PR labeling step in agent prompts** — both the implement and retry agent prompts now include a step to fetch all repo labels via the REST API (`gh api repos/.../labels`) and apply up to 3 matching labels to the PR; labeling failures are silently skipped so they never block the task
...
### Changed
- **Agent restart endpoint refactored** — `POST /api/agents/{id}/restart` now accepts agents in `running`, `failed`, `stopped`, and `timeout` statuses; the kill/cleanup logic is skipped for already-dead agents; old worktree is cleaned up before dispatching the new agent (not after) to avoid destroying the new agent's working directory
- **Issue status reset scoped to running agents** — restarting a dead agent no longer resets its issue to `pending`, preventing duplicate dispatch from the issue watcher
