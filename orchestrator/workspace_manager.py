...
def ensure_default_workspace() -> dict | None:
    """Create a default workspace from .env values if no workspaces exist.

    Returns the default workspace dict, or None if env vars aren't configured.
    """
    existing = db.get_all_workspaces()
    if existing:
        return None

    # Import legacy config values
    from orchestrator.config import GITHUB_REPO, TARGET_REPO_PATH, BASE_BRANCH

    if GITHUB_REPO == "owner/repo":
        return None  # Not configured

    if not Path(str(TARGET_REPO_PATH)).exists():
        return None
    ...
    # Backfill existing issues/agents/pr_reviews with default workspace_id
    conn = db._get_connection()
    conn.execute("UPDATE issues SET workspace_id = ? WHERE workspace_id IS NULL", (workspace_id,))
    conn.execute("UPDATE agents SET workspace_id = ? WHERE workspace_id IS NULL", (workspace_id,))
    conn.execute("UPDATE pr_reviews SET workspace_id = ? WHERE workspace_id IS NULL", (workspace_id,))
    conn.commit()

    logger.info("Created default workspace '%s' from .env config", name)
    return db.get_workspace(workspace_id)
